...
def load_data():
    ...
    for league_name, file_name in league_files.items():
        try:
            ...
            st.sidebar.error(f"Error: File '{file_name}' not found.")
            files_found = False
        except pd.errors.EmptyDataError:
            st.sidebar.error(f"Error: File '{file_name}' is empty or corrupted.")
            files_found = False
    ...
    if not all_data:
        st.sidebar.error("No data could be loaded from any files.")
        return pd.DataFrame()
    
    try:
        combined_df = pd.concat(all_data, ignore_index=True)
    except Exception as e:
        st.sidebar.error(f"Error combining dataframes: {str(e)}")
        return pd.DataFrame()
    ...
    try:
        ...
        st.sidebar.error(f"Error during data cleaning: {str(e)}")
        return pd.DataFrame()